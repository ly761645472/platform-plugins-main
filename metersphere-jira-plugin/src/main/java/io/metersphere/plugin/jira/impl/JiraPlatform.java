...
public class JiraPlatform extends AbstractPlatform {
    ...
    public void validateProjectConfig(String projectConfigStr) {
        try {
            ...
            if (StringUtils.isBlank(projectConfig.getJiraKey())) {
                throw new MSPluginException("JIRA项目Key不能为空!");
            }
        ...
        if (StringUtils.isBlank(issueKey)) {
            ...
            if (!CollectionUtils.isEmpty(issueTypeStatus)) {
                issueTypeStatus.forEach(item -> item.forEach(statuses -> {
                    ...
                    option.setText(statuses.getUntranslatedName().toUpperCase());
    ...
    public List<SelectOption> getStatusTransitionsLastSteps(String projectConfig) {
        List<SelectOption> statusOptions = new ArrayList<>();
        JiraProjectConfig config = getProjectConfig(projectConfig);
        String luckyKey = getLuckyKey(config);
        if (StringUtils.isEmpty(luckyKey)) {
            return new ArrayList<>();
        }
        List<JiraTransitionsResponse.Transitions> transitions = jiraClient.getTransitions(luckyKey);
        if (!CollectionUtils.isEmpty(transitions)) {
            transitions.forEach(item -> {
                if (StringUtils.equals(item.getTo().getStatusCategory().getKey(), "done")) {
                    SelectOption option = new SelectOption();
                    option.setText(item.getTo().getName());
                    option.setValue(item.getId());
                    statusOptions.add(option);
    ...
    public PlatformBugUpdateDTO updateBug(PlatformBugUpdateRequest request) {
        ...
        new Thread(() -> {
            ...
            if (!CollectionUtils.isEmpty(request.getRichFileMap())) {
                // add image attachment
                request.getRichFileMap().values().forEach(file -> jiraClient.uploadAttachment(request.getPlatformBugId(), file));
            }

            // link or unlink issue
            if (!CollectionUtils.isEmpty(issueLinkFields)) {
                // When edited, clean up all the last issue links and re-associate a group of issue links
    ...
    public void deleteBug(PlatformBugDeleteRequest request) {
        jiraClient.deleteIssue(request.getPlatformBugKey());
    }
    ...
    private List<Map<String, Object>> queryAllDemand(DemandPageRequest request) {
        ...
        List<Map<String, Object>> allIssues = new ArrayList<>();
        // Jira 接口限制，一次最多返回 5000 条
        int maxResult = 5000;
        // 第一页 nextPageToken = null
        String nextPageToken = null;

        do {
            // 核心调用：传入 nextPageToken，startAt 已失效，随便传 0 即可
            Map<String, Object> bodyMap = jiraClient.pageDemand2(
                    projectConfig.getJiraKey(),
                    projectConfig.getJiraDemandTypeId(),
                    0,
                    maxResult,
                    null,
                    nextPageToken
            ...
            Boolean isLast = (Boolean) bodyMap.get("isLast");
            nextPageToken = (String) bodyMap.get("nextPageToken");
    ...
    private void setSpecialParam(Map<String, Object> fields) {
        try {
            ...
            PluginLogUtils.error(e);
            throw new MSPluginException("解析Jira特殊参数失败: " + e.getMessage());
        }
    }
     ...
     * @param result                 同步缺陷结果
     * @param msBug                  平台缺陷
     * @param jiraIssueAttachmentMap 附件集合
     */
     ...
     * @param platformCustomFieldMap    平台自定义字段集合
    ...
    private String parseMsRichTextToJira(String key, String content, Map<String, File> msFileMap, Map<String, String> platformCustomFieldMap, List<String> remainImgNameFromRichText) {
        ...
        return parseHtmlTagToJiraRichText(jiraRichText.toString());
    }
     ...
     * 处理MS富文本内容 => Jira富文本内容
     *
     * @param htmlContent MS富文本内容
     * @return Jira富文本内容
     */
    private String parseHtmlTagToJiraRichText(String htmlContent) {
        if (StringUtils.isEmpty(htmlContent)) {
            return null;
        }
        StringBuilder jiraRichText = new StringBuilder();
        /**
         * 暂时只处理MS富文本编辑器中通用的标签 => Jira富文本语法
         * 支持标签: p, h1, h2, h3, h4, h5, h6, strong(p), em(p), u(p), s(p), span(color)
         */
        Document parse = Jsoup.parse(htmlContent);
        Element body = parse.body();
        Elements childrens = body.children();
        for (Element children : childrens) {
            // 根据要求后续补充其余标签
            if (StringUtils.isEmpty(children.text())) {
                // 标签内容为空
                continue;
            }
            if (children.select("strong").size() > 0) {
                // 处理加粗
                jiraRichText.append("*").append(children.text()).append("* \n\n ");
            } else if (children.select("em").size() > 0) {
                // 处理斜体
                jiraRichText.append("_").append(children.text()).append("_ \n\n ");
            } else if (children.select("u").size() > 0) {
                // 处理下划线
                jiraRichText.append("+").append(children.text()).append("+ \n\n ");
            } else if (children.select("s").size() > 0) {
                // 处理删除线
                jiraRichText.append("-").append(children.text()).append("- \n\n");
            } else if (StringUtils.equals(children.tagName(), "span")) {
                // 处理字体颜色
                String color = children.attr("color");
                jiraRichText.append("{color:").append(color).append("}").append(children.text()).append("{color} \n\n");
            } else if (StringUtils.contains(children.tagName(), "h")) {
                // 处理标题
                jiraRichText.append("h").append(children.tagName().substring(1)).append(". ").append(children.text()).append(" \n\n ");
            } else if (StringUtils.equals(children.tagName(), "p")) {
                // 处理段落
                jiraRichText.append(children.text()).append(" \n\n ");
            }
        }
    ...
    private String getLuckyKey(JiraProjectConfig projectConfig) {
        ...
        return jiraIssues.get(0).getKey();
    }
}
